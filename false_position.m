...
% false position calculation for the first x_r
x_r(i,1) = x_u(i,1) - (thefunc(x_u(i,1))/(thefunc(x_l(i,1)) - thefunc(x_u(i,1))))*(x_l(i,1) - x_u(i,1)); 
...
while abs(Ea) >= Es
    ...
    % false position calculation for x_r
    x_r(i,1) = x_u(i,1) - (thefunc(x_u(i,1))/(thefunc(x_l(i,1)) - thefunc(x_u(i,1))))*(x_l(i,1) - x_u(i,1));
